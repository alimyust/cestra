...
class DiffDriveInterface(Node):
    def __init__(self):
        super().__init__('diff_drive_interface')
        self.get_logger().info('DiffDriveInterface node has been started.')

        self.wheel_radius = 0.05  # meters
        self.wheel_base = 0.19    # meters

        self.left_wheel_publisher_ = self.create_publisher(Float64, 'left_wheel_cmd', 10)
        self.right_wheel_publisher_ = self.create_publisher(Float64, 'right_wheel_cmd', 10)
...
def main(args=None):
    ...
    diff_drive_interface = DiffDriveInterface()
    rclpy.spin(diff_drive_interface)

    diff_drive_interface.destroy_node()
    rclpy.shutdown()    
